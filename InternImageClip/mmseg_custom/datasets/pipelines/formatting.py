...
class DefaultFormatBundle(object):
    ...
    def __call__(self, results):
        ...
        if 'pair_img' in results:
            pair_img = results['pair_img']
            if len(pair_img.shape) < 3:
                pair_img = np.expand_dims(pair_img, -1)
            pair_img = np.ascontiguousarray(pair_img.transpose(2, 0, 1))
            results['pair_img'] = DC(to_tensor(pair_img), stack=True)
