...
weather_texts = ["rainy weather, rain", "snowy weather, snow",
                 "foggy weather, fog",
                 "clear weather, clear", "sunny weather, sun",
                 "cloudy weather, cloud",
                 "overcast weather, overcast clouds",
                 "partly cloudy weather, partly cloudy",
                 "misty weather, mist", "hazy weather, haze",
                 "downpour weather, downpour rain",
    ...
    def __init__(self,
        ...
        if post_norm_block_ids is not None:  # for InternImage-H/G
            self.post_norms = nn.ModuleList(
                [build_norm_layer(channels, 'LN', eps=1e-6) for _ in
                 post_norm_block_ids]
            )
    ...
    def forward(self, x, clip, return_wo_downsample=False):
        ...
        if self.queries_mlp is not None:
            ...
            if queries.shape[2] != self.query_size:
                queries = F.adaptive_avg_pool2d(queries, (
                self.query_size, self.query_size))
            ...
            queries = queries.view(x.shape[0], x.shape[3], self.query_size,
                                   self.query_size)
...
class InternImageNaClip(nn.Module):
    r""" InternImage
    ...
    def __init__(self,
        ...
        self.clip_model, _ = clip.load(
            '/home/weijn/Project/InternImage-master/ViT-B-32.pt',
            device=self.device, jit=False)
        self.clip_model.visual.set_params('reduced','naclip',5.)
        for param in self.clip_model.parameters():
            param.requires_grad = False
        query_features = []
        for weather in weather_texts:
            query = clip.tokenize(
                [temp(weather) for temp in openai_imagenet_template]).to(
                self.device)
        ...
        self.clip_text_encoded = torch.cat(query_features, dim=0)
        # self.clip_text_encoded = self.clip_model.encode_text(self.clip_text)
        self.clip_mlp = MLPLayer(512, len(weather_texts), len(weather_texts),
                                 drop=0.2)
        ...
        self.query_mlp_1 = nn.Linear(3136, 1024, dtype=torch.float32,
                                     device=self.device)
        self.query_mlp_2 = nn.Linear(784, 1024, dtype=torch.float32,
                                     device=self.device)
        self.query_mlp_3 = nn.Linear(196, 1024, dtype=torch.float32,
                                     device=self.device)
        self.qurery_size = [56, 28, 14, None]
        self.query_mlp_list = [self.query_mlp_1, self.query_mlp_2,
                               self.query_mlp_3, None]
        self.crossattention_1 = AttentiveBlock(1024, 8, out_dim=3136, drop=0.1,
                                               attn_drop=0.1, drop_path=0.1)
        self.crossattention_2 = AttentiveBlock(1024, 8, out_dim=784, drop=0.1,
                                               attn_drop=0.1, drop_path=0.1)
        self.crossattention_3 = AttentiveBlock(1024, 8, out_dim=196, drop=0.1,
                                               attn_drop=0.1, drop_path=0.1)
        self.crossattention_list = [self.crossattention_1,
                                    self.crossattention_2,
                                    self.crossattention_3, None]
    ...
    def clip_slide(self,img,stride=224, crop_size=224):
        """
        ...
        preds = img.new_zeros((batch_size, 512))
        ...
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                crop_seg_logit = self.clip_model.encode_image(crop_img,return_all=False)
    ...
    def forward(self, x):
        clip_img_encoded = self.clip_slide(x)
        # if x.size(2) != 224 or x.size(3) != 224:
        #     resized_x = F.interpolate(x.clone(), size=(224, 224),
        #                               mode='bilinear', align_corners=False)
        #     clip_img_encoded = self.clip_model.encode_image(resized_x,return_all=False)
        # else:
        #     clip_img_encoded = self.clip_model.encode_image(x,return_all=False)
        clip_img_encoded /= clip_img_encoded.norm(dim=-1, keepdim=True)
        ...
        adverse_weights = self.clip_mlp(clip_img_encoded).unsqueeze(2).repeat(1,
                                                                              1,
                                                                              512)
        weathered_features = clip_text_encoded.unsqueeze(0).repeat(x.shape[0],
                                                                   1, 1)
        ...
        clip_vector = torch.cat(
            [clip_img_encoded.unsqueeze(1), weathered_features],
            dim=2)
        ...
        for level_idx, level in enumerate(self.levels):
            x, x_ = level(x, clip_vector, return_wo_downsample=True)
